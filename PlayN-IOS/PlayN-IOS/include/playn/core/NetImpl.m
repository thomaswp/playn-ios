...
withPlaynCoreUtilCallback:(id<PlaynCoreUtilCallback>)callback {
  [((id<PlaynCoreNet_Builder>) nil_chk([self reqWithNSString:url])) executeWithPlaynCoreUtilCallback:[self adaptWithPlaynCoreUtilCallback:callback]];
}

- (void)postWithNSString:(NSString *)url
            withNSString:(NSString *)data
withPlaynCoreUtilCallback:(id<PlaynCoreUtilCallback>)callback {
  [((id<PlaynCoreNet_Builder>) nil_chk([((id<PlaynCoreNet_Builder>) nil_chk([self reqWithNSString:url])) setPayloadWithNSString:data])) executeWithPlaynCoreUtilCallback:[self adaptWithPlaynCoreUtilCallback:callback]];
}

- (id<PlaynCoreNet_Builder>)reqWithNSString:(NSString *)url {
  return [[PlaynCoreNetImpl_BuilderImpl alloc] initWithPlaynCoreNetImpl:self withNSString:url];
}
...
- (id<PlaynCoreUtilCallback>)adaptWithPlaynCoreUtilCallback:(id<PlaynCoreUtilCallback>)callback {
  return [[PlaynCoreNetImpl_$1 alloc] initWithPlaynCoreUtilCallback:callback withPlaynCoreUtilCallback:callback];
}
...
- (id)initWithPlaynCoreNetImpl:(PlaynCoreNetImpl *)outer$
  ...
  if ((self = [super init])) {
    ...
    contentType__ = @"text/plain";
    [PlaynCoreAsserts checkArgumentWithBOOL:[((NSString *) nil_chk(url)) hasPrefix:@"http:"] || [((NSString *) nil_chk(url)) hasPrefix:@"https:"] withId:@"Only http and https URLs are supported"];
    self.url = url;
  }
  return self;
}
...
- (id<PlaynCoreNet_Builder>)setPayloadWithByteArray:(IOSByteArray *)payload {
  return [self setPayloadWithByteArray:payload withNSString:@"application/octet-stream"];
}
...
- (id<PlaynCoreNet_Builder>)addHeaderWithNSString:(NSString *)name
                                     withNSString:(NSString *)value {
  [((id<JavaUtilList>) nil_chk(headers_)) addWithId:[[PlaynCoreNetImpl_Header alloc] initWithPlaynCoreNetImpl:this$0_ withNSString:name withNSString:value]];
  return self;
}
...
- (id<JavaLangIterable>)headerNames {
  return [((id<JavaUtilMap>) nil_chk([self headers])) keySet];
}

- (NSString *)headerWithNSString:(NSString *)name {
  id<JavaUtilList> values = [((id<JavaUtilMap>) nil_chk([self headers])) getWithId:name];
  return (values == nil) ? nil : [((id<JavaUtilList>) nil_chk(values)) getWithInt:0];
}

- (id<JavaUtilList>)headersWithNSString:(NSString *)name {
  id<JavaUtilList> values = [((id<JavaUtilMap>) nil_chk([self headers])) getWithId:name];
  return values == nil ? [JavaUtilCollections emptyList] : values;
}

- (id<JavaUtilMap>)headers {
  if (headersMap_ == nil) {
    headersMap_ = [self extractHeaders];
  }
  return headersMap_;
}
...
- (void)onSuccessWithId:(id<PlaynCoreNet_Response>)rsp {
  if ([((id<PlaynCoreNet_Response>) nil_chk(rsp)) responseCode] == 200) {
    ...
    [((id<PlaynCoreUtilCallback>) nil_chk(val$callback_)) onFailureWithJavaLangThrowable:[[PlaynCoreNet_HttpException alloc] initWithInt:[((id<PlaynCoreNet_Response>) nil_chk(rsp)) responseCode] withNSString:[((id<PlaynCoreNet_Response>) nil_chk(rsp)) payloadString]]];
  }
}
