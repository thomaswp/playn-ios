//
//  Generated by the J2ObjC translator.  DO NOT EDIT!
//  source: playn/core/Platform.java
//
//  Created by Thomas on 7/25/13.
//

#import "JreEmulation.h"

#if !PlaynCorePlatform_RESTRICT
#define PlaynCorePlatform_INCLUDE_ALL 1
#endif
#undef PlaynCorePlatform_RESTRICT

#if !defined (_PlaynCorePlatform_) && (PlaynCorePlatform_INCLUDE_ALL || PlaynCorePlatform_INCLUDE)
#define _PlaynCorePlatform_

@class PlaynCorePlatform_TypeEnum;
@protocol JavaLangRunnable;
@protocol PlaynCoreAnalytics;
@protocol PlaynCoreAssets;
@protocol PlaynCoreAudio;
@protocol PlaynCoreGame;
@protocol PlaynCoreGraphics;
@protocol PlaynCoreJson;
@protocol PlaynCoreKeyboard;
@protocol PlaynCoreLog;
@protocol PlaynCoreMouse;
@protocol PlaynCoreNet;
@protocol PlaynCorePlayN_LifecycleListener;
@protocol PlaynCorePointer;
@protocol PlaynCoreRegularExpression;
@protocol PlaynCoreStorage;
@protocol PlaynCoreTouch;
@protocol PlaynCoreUIOverlay;

@protocol PlaynCorePlatform < NSObject, JavaObject >
- (void)runWithPlaynCoreGame:(id<PlaynCoreGame>)game;
- (PlaynCorePlatform_TypeEnum *)type;
- (double)time;
- (int)tick;
- (float)random;
- (void)openURLWithNSString:(NSString *)url;
- (void)invokeLaterWithJavaLangRunnable:(id<JavaLangRunnable>)runnable;
- (void)setLifecycleListenerWithPlaynCorePlayN_LifecycleListener:(id<PlaynCorePlayN_LifecycleListener>)listener;
- (void)setPropagateEventsWithBOOL:(BOOL)propagate;
- (id<PlaynCoreAudio>)audio;
- (id<PlaynCoreGraphics>)graphics;
- (id<PlaynCoreAssets>)assets;
- (id<PlaynCoreJson>)json;
- (id<PlaynCoreKeyboard>)keyboard;
- (id<PlaynCoreLog>)log;
- (id<PlaynCoreNet>)net;
- (id<PlaynCorePointer>)pointer;
- (id<PlaynCoreMouse>)mouse;
- (id<PlaynCoreTouch>)touch;
- (id<PlaynCoreStorage>)storage;
- (id<PlaynCoreAnalytics>)analytics;
- (id<PlaynCoreRegularExpression>)regularExpression;
- (id<PlaynCoreUIOverlay>)uiOverlay;
@end
#endif

#if !defined (_PlaynCorePlatform_TypeEnum_) && (PlaynCorePlatform_INCLUDE_ALL || PlaynCorePlatform_TypeEnum_INCLUDE)
#define _PlaynCorePlatform_TypeEnum_

#define JavaLangEnum_RESTRICT 1
#define JavaLangEnum_INCLUDE 1
#include "java/lang/Enum.h"

typedef enum {
  PlaynCorePlatform_Type_JAVA = 0,
  PlaynCorePlatform_Type_HTML = 1,
  PlaynCorePlatform_Type_ANDROID = 2,
  PlaynCorePlatform_Type_IOS = 3,
  PlaynCorePlatform_Type_FLASH = 4,
  PlaynCorePlatform_Type_STUB = 5,
} PlaynCorePlatform_Type;

@interface PlaynCorePlatform_TypeEnum : JavaLangEnum < NSCopying > {
}
+ (PlaynCorePlatform_TypeEnum *)JAVA;
+ (PlaynCorePlatform_TypeEnum *)HTML;
+ (PlaynCorePlatform_TypeEnum *)ANDROID;
+ (PlaynCorePlatform_TypeEnum *)IOS;
+ (PlaynCorePlatform_TypeEnum *)FLASH;
+ (PlaynCorePlatform_TypeEnum *)STUB;
+ (IOSObjectArray *)values;
+ (PlaynCorePlatform_TypeEnum *)valueOfWithNSString:(NSString *)name;
- (id)copyWithZone:(NSZone *)zone;
- (id)initWithNSString:(NSString *)name withInt:(int)ordinal;
@end
#endif
