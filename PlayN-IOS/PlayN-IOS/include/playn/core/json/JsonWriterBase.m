...
- (id)objectWithNSString:(NSString *)key
  ...
  else (void) [self objectWithNSString:key];
  {
    ...
    while ([((id<JavaUtilIterator>) NIL_CHK(iter__)) hasNext]) {
      ...
      if (!([[((id<JavaUtilMap_Entry>) NIL_CHK(entry)) getKey] isKindOfClass:[NSString class]])) @throw [[PlaynCoreJsonJsonWriterException alloc] initWithNSString:[NSString stringWithFormat:@"Invalid key type for map: %@", ([((id<JavaUtilMap_Entry>) NIL_CHK(entry)) getKey] == nil ? @"null" : [[((id<JavaUtilMap_Entry>) NIL_CHK(entry)) getKey] getClass])]];
...
+ (NSString *)escapeWithNSString:(NSString *)s {
  NSString *json = [((id<PlaynCoreJson_Writer>) ((id<PlaynCoreJson_Writer>) [[[PlaynCoreJsonJsonStringWriter alloc] init] valueWithNSString:s])) write];
