...
- (id)initWithPlaynCoreGlGLContext:(PlaynCoreGlGLContext *)ctx {
  if ((self = [super initWithPlaynCoreGlGLContext:ctx])) {
    ...
    if (maxVecs < [self vec4sPerQuad]) @throw [[JavaLangRuntimeException alloc] initWithNSString:[NSString stringWithFormat:@"GL_MAX_VERTEX_UNIFORM_VECTORS too low: have %d, need at least %d", maxVecs, [self vec4sPerQuad]]];
...
- (NSString *)vertexShader {
  return [((NSString *) nil_chk([((NSString *) nil_chk([self baseVertexShader])) replace:@"_MAX_QUADS_" withSequence:[NSString stringWithFormat:@"%d", maxQuads_]])) replace:@"_VEC4S_PER_QUAD_" withSequence:[NSString stringWithFormat:@"%d", [self vec4sPerQuad]]];
}
...
- (PlaynCoreGlGLShader_Core *)createTextureCore {
  return [[PlaynCoreGlQuadShader_QuadCore alloc] initWithPlaynCoreGlQuadShader:self withNSString:[self vertexShader] withNSString:[self textureFragmentShader]];
}

- (PlaynCoreGlGLShader_Core *)createColorCore {
  return [[PlaynCoreGlQuadShader_QuadCore alloc] initWithPlaynCoreGlQuadShader:self withNSString:[self vertexShader] withNSString:[self colorFragmentShader]];
}
...
- (id)initWithPlaynCoreGlQuadShader:(PlaynCoreGlQuadShader *)outer$
  ...
  if ((self = [super initWithPlaynCoreGlGLShader:outer$ withNSString:vertShader withNSString:fragShader])) {
    this$1_ = outer$;
    data_ = [((PlaynCoreGlGLContext *) nil_chk(outer$.ctx)) createFloatBufferWithInt:outer$.maxQuads * [outer$ vec4sPerQuad] * 4];
    uScreenSize_ = [((id<PlaynCoreGlGLProgram>) nil_chk(prog_)) getUniform2fWithNSString:@"u_ScreenSize"];
    uData_ = [((id<PlaynCoreGlGLProgram>) nil_chk(prog_)) getUniform4fvWithNSString:@"u_Data"];
    aVertex_ = [((id<PlaynCoreGlGLProgram>) nil_chk(prog_)) getAttribWithNSString:@"a_Vertex" withInt:PlaynCoreGlQuadShader_VERTEX_SIZE withInt:PlaynCoreGlGL20_GL_SHORT];
    vertices_ = [((PlaynCoreGlGLContext *) nil_chk(outer$.ctx)) createShortBufferWithInt:outer$.maxQuads * PlaynCoreGlQuadShader_VERTICES_PER_QUAD * PlaynCoreGlQuadShader_VERTEX_SIZE];
    elements_ = [((PlaynCoreGlGLContext *) nil_chk(outer$.ctx)) createShortBufferWithInt:outer$.maxQuads * PlaynCoreGlQuadShader_ELEMENTS_PER_QUAD];
    for (int ii = 0; ii < outer$.maxQuads; ii++) {
      (void) [((id<PlaynCoreGlGLBuffer_Short>) nil_chk([((id<PlaynCoreGlGLBuffer_Short>) nil_chk(vertices_)) addWithInt:0 withInt:0])) addWithInt:ii];
      (void) [((id<PlaynCoreGlGLBuffer_Short>) nil_chk([((id<PlaynCoreGlGLBuffer_Short>) nil_chk(vertices_)) addWithInt:1 withInt:0])) addWithInt:ii];
      (void) [((id<PlaynCoreGlGLBuffer_Short>) nil_chk([((id<PlaynCoreGlGLBuffer_Short>) nil_chk(vertices_)) addWithInt:0 withInt:1])) addWithInt:ii];
      (void) [((id<PlaynCoreGlGLBuffer_Short>) nil_chk([((id<PlaynCoreGlGLBuffer_Short>) nil_chk(vertices_)) addWithInt:1 withInt:1])) addWithInt:ii];
      int base = ii * PlaynCoreGlQuadShader_VERTICES_PER_QUAD;
      (void) [((id<PlaynCoreGlGLBuffer_Short>) nil_chk([((id<PlaynCoreGlGLBuffer_Short>) nil_chk([((id<PlaynCoreGlGLBuffer_Short>) nil_chk(elements_)) addWithInt:base + 0])) addWithInt:base + 1])) addWithInt:base + 2];
      (void) [((id<PlaynCoreGlGLBuffer_Short>) nil_chk([((id<PlaynCoreGlGLBuffer_Short>) nil_chk([((id<PlaynCoreGlGLBuffer_Short>) nil_chk(elements_)) addWithInt:base + 1])) addWithInt:base + 3])) addWithInt:base + 2];
    }
