...
- (NSString *)computePathWithNSString:(NSString *)path
  ...
  int didx = [((NSString *) nil_chk(path)) lastIndexOfString:@"."];
  if (didx == -1) {
    return path;
  }
  else {
    return [NSString stringWithFormat:@"%@@%dx%@", [((NSString *) nil_chk(path)) substring:0 endIndex:didx], scaleFactor, [((NSString *) nil_chk(path)) substring:didx]];
  }
}
