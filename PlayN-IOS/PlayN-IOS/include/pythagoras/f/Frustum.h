//
//  Generated by the J2ObjC translator.  DO NOT EDIT!
//  source: pythagoras/f/Frustum.java
//
//  Created by Thomas on 7/25/13.
//

#import "JreEmulation.h"

#if !PythagorasFFrustum_RESTRICT
#define PythagorasFFrustum_INCLUDE_ALL 1
#endif
#undef PythagorasFFrustum_RESTRICT

#if !defined (_PythagorasFFrustum_) && (PythagorasFFrustum_INCLUDE_ALL || PythagorasFFrustum_INCLUDE)
#define _PythagorasFFrustum_

@class IOSObjectArray;
@class PythagorasFBox;
@class PythagorasFFrustum_IntersectionTypeEnum;
@class PythagorasFMatrix3;
@class PythagorasFVector3;
@protocol PythagorasFIVector3;

@interface PythagorasFFrustum : NSObject {
 @public
  IOSObjectArray *_vertices_;
  IOSObjectArray *_planes_;
  PythagorasFBox *_bounds_;
}

@property (nonatomic, strong) IOSObjectArray *_vertices;
@property (nonatomic, strong) IOSObjectArray *_planes;
@property (nonatomic, strong) PythagorasFBox *_bounds;

+ (PythagorasFVector3 *)_vertex;
+ (void)set_vertex:(PythagorasFVector3 *)_vertex;
- (id)init;
- (IOSObjectArray *)vertices;
- (PythagorasFBox *)bounds;
- (PythagorasFFrustum *)setToPerspectiveWithFloat:(float)fovy
                                        withFloat:(float)aspect
                                        withFloat:(float)znear
                                        withFloat:(float)zfar;
- (PythagorasFFrustum *)setToFrustumWithFloat:(float)left
                                    withFloat:(float)right
                                    withFloat:(float)bottom
                                    withFloat:(float)top
                                    withFloat:(float)near
                                    withFloat:(float)far;
- (PythagorasFFrustum *)setToOrthoWithFloat:(float)left
                                  withFloat:(float)right
                                  withFloat:(float)bottom
                                  withFloat:(float)top
                                  withFloat:(float)near
                                  withFloat:(float)far;
- (PythagorasFFrustum *)setToProjectionWithFloat:(float)left
                                       withFloat:(float)right
                                       withFloat:(float)bottom
                                       withFloat:(float)top
                                       withFloat:(float)near
                                       withFloat:(float)far
                         withPythagorasFIVector3:(id<PythagorasFIVector3>)nearFarNormal
                                        withBOOL:(BOOL)ortho
                                        withBOOL:(BOOL)mirrored;
- (float)distanceWithPythagorasFVector3:(PythagorasFVector3 *)point;
- (PythagorasFFrustum_IntersectionTypeEnum *)intersectionTypeWithPythagorasFBox:(PythagorasFBox *)box;
- (PythagorasFBox *)boundsUnderRotationWithPythagorasFMatrix3:(PythagorasFMatrix3 *)matrix
                                           withPythagorasFBox:(PythagorasFBox *)result;
- (void)updateDerivedState;
@end
#endif

#if !defined (_PythagorasFFrustum_IntersectionTypeEnum_) && (PythagorasFFrustum_INCLUDE_ALL || PythagorasFFrustum_IntersectionTypeEnum_INCLUDE)
#define _PythagorasFFrustum_IntersectionTypeEnum_

#define JavaLangEnum_RESTRICT 1
#define JavaLangEnum_INCLUDE 1
#include "java/lang/Enum.h"

typedef enum {
  PythagorasFFrustum_IntersectionType_NONE = 0,
  PythagorasFFrustum_IntersectionType_INTERSECTS = 1,
  PythagorasFFrustum_IntersectionType_CONTAINS = 2,
} PythagorasFFrustum_IntersectionType;

@interface PythagorasFFrustum_IntersectionTypeEnum : JavaLangEnum < NSCopying > {
}
+ (PythagorasFFrustum_IntersectionTypeEnum *)NONE;
+ (PythagorasFFrustum_IntersectionTypeEnum *)INTERSECTS;
+ (PythagorasFFrustum_IntersectionTypeEnum *)CONTAINS;
+ (IOSObjectArray *)values;
+ (PythagorasFFrustum_IntersectionTypeEnum *)valueOfWithNSString:(NSString *)name;
- (id)copyWithZone:(NSZone *)zone;
- (id)initWithNSString:(NSString *)name withInt:(int)ordinal;
@end
#endif
