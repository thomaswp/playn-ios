...
- (id)init {
  if ((self = [super init])) {
    _vertices_ = [[IOSObjectArray alloc] initWithLength:8 type:[IOSClass classWithClass:[PythagorasFVector3 class]]];
    _planes_ = [[IOSObjectArray alloc] initWithLength:6 type:[IOSClass classWithClass:[PythagorasFPlane class]]];
    _bounds_ = [[PythagorasFBox alloc] init];
    for (int ii = 0; ii < 8; ii++) {
      [((IOSObjectArray *) NIL_CHK(_vertices_)) replaceObjectAtIndex:ii withObject:[[PythagorasFVector3 alloc] init]];
    }
    for (int ii = 0; ii < 6; ii++) {
      [((IOSObjectArray *) NIL_CHK(_planes_)) replaceObjectAtIndex:ii withObject:[[PythagorasFPlane alloc] init]];
    }
  }
  return self;
}
