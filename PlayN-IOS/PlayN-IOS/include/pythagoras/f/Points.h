//
//  Generated by the J2ObjC translator.  DO NOT EDIT!
//  source: pythagoras/f/Points.java
//
//  Created by Thomas on 7/25/13.
//

#import "JreEmulation.h"

#if !PythagorasFPoints_RESTRICT
#define PythagorasFPoints_INCLUDE_ALL 1
#endif
#undef PythagorasFPoints_RESTRICT

#if !defined (_PythagorasFPoints_) && (PythagorasFPoints_INCLUDE_ALL || PythagorasFPoints_INCLUDE)
#define _PythagorasFPoints_

@class PythagorasFPoint;
@protocol PythagorasFIPoint;

@interface PythagorasFPoints : NSObject {
}

+ (id<PythagorasFIPoint>)ZERO;
+ (float)distanceSqWithFloat:(float)x1
                   withFloat:(float)y1
                   withFloat:(float)x2
                   withFloat:(float)y2;
+ (float)distanceWithFloat:(float)x1
                 withFloat:(float)y1
                 withFloat:(float)x2
                 withFloat:(float)y2;
+ (BOOL)epsilonEqualsWithPythagorasFIPoint:(id<PythagorasFIPoint>)p1
                     withPythagorasFIPoint:(id<PythagorasFIPoint>)p2;
+ (BOOL)epsilonEqualsWithPythagorasFIPoint:(id<PythagorasFIPoint>)p1
                     withPythagorasFIPoint:(id<PythagorasFIPoint>)p2
                                 withFloat:(float)epsilon;
+ (PythagorasFPoint *)transformWithFloat:(float)x
                               withFloat:(float)y
                               withFloat:(float)sx
                               withFloat:(float)sy
                               withFloat:(float)rotation
                               withFloat:(float)tx
                               withFloat:(float)ty
                    withPythagorasFPoint:(PythagorasFPoint *)result;
+ (PythagorasFPoint *)transformWithFloat:(float)x
                               withFloat:(float)y
                               withFloat:(float)sx
                               withFloat:(float)sy
                               withFloat:(float)sina
                               withFloat:(float)cosa
                               withFloat:(float)tx
                               withFloat:(float)ty
                    withPythagorasFPoint:(PythagorasFPoint *)result;
+ (PythagorasFPoint *)inverseTransformWithFloat:(float)x
                                      withFloat:(float)y
                                      withFloat:(float)sx
                                      withFloat:(float)sy
                                      withFloat:(float)rotation
                                      withFloat:(float)tx
                                      withFloat:(float)ty
                           withPythagorasFPoint:(PythagorasFPoint *)result;
+ (NSString *)pointToStringWithFloat:(float)x
                           withFloat:(float)y;
- (id)init;
@end
#endif
