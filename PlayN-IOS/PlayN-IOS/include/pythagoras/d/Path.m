...
- (id)initPythagorasDPathWithInt:(int)rule
  ...
  if ((self = [super init])) {
    ...
    points_ = [[IOSDoubleArray alloc] initWithLength:initialCapacity * 2];
  }
  return self;
}
...
- (PythagorasDPath *)clone {
  return [[PythagorasDPath alloc] initWithInt:rule_ withJavaLangByteArray:[PythagorasUtilPlatform cloneWithJavaLangByteArray:types_] withJavaLangDoubleArray:[PythagorasUtilPlatform cloneWithJavaLangDoubleArray:points_] withInt:typeSize_ withInt:pointSize_];
}
...
- (void)checkBufWithInt:(int)pointCount
  ...
  if (typeSize_ == (int) [((IOSByteArray *) NIL_CHK(types_)) count]) {
    IOSByteArray *tmp = [[IOSByteArray alloc] initWithLength:typeSize_ + PythagorasDPath_BUFFER_CAPACITY];
  ...
  if (pointSize_ + pointCount > (int) [((IOSDoubleArray *) NIL_CHK(points_)) count]) {
    IOSDoubleArray *tmp = [[IOSDoubleArray alloc] initWithLength:pointSize_ + [JavaLangMath maxWithInt:PythagorasDPath_BUFFER_CAPACITY * 2 withInt:pointCount]];
