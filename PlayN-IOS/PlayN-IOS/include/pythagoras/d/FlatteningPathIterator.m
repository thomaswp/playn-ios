...
- (id)initPythagorasDFlatteningPathIteratorWithPythagorasDPathIterator:(id<PythagorasDPathIterator>)path
  ...
  if ((self = [super init])) {
    bufEmpty_ = YES;
    coords_ = [[IOSDoubleArray alloc] initWithLength:6];
    if (flatness < 0) {
    ...
    self.buf = [[IOSDoubleArray alloc] initWithLength:bufSize_];
...
- (void)evaluate {
  ...
  switch (bufType_) {
    ...
    while (bufSubdiv_ < bufLimit_) {
      ...
      if (bufIndex_ <= 4) {
        IOSDoubleArray *tmp = [[IOSDoubleArray alloc] initWithLength:bufSize_ + PythagorasDFlatteningPathIterator_BUFFER_CAPACITY];
