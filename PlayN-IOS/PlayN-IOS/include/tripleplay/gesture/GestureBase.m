...
- (void)setStateWithTripleplayGestureGesture_StateEnum:(TripleplayGestureGesture_StateEnum *)state {
  ...
  if (state == [TripleplayGestureGesture_StateEnum GREEDY] && _state_ != [TripleplayGestureGesture_StateEnum PASSIVE]) [((TripleplayUtilLogger *) nil_chk([TripleplayGestureLog log])) warningWithNSString:@"Transitioning to GREEDY from !PASSIVE" withNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ @"current", _state_, @"gesture", self } count:4 type:[IOSClass classWithClass:[NSObject class]]]];
...
- (id)init {
  if ((self = [super init])) {
    ...
    _completed_ = [ReactSignal create];
  }
  return self;
}
