...
- (void)pushWithJavaLangIterable:(id<JavaLangIterable>)screens
withTripleplayGameScreenStack_Transition:(id<TripleplayGameScreenStack_Transition>)trans {
  if (![((id<JavaUtilIterator>) nil_chk([((id<JavaLangIterable>) nil_chk(screens)) iterator])) hasNext]) {
    @throw [[JavaLangIllegalArgumentException alloc] initWithNSString:@"Cannot push empty list of screens."];
  }
  if ([((id<JavaUtilList>) nil_chk(_screens_)) isEmpty]) {
    {
      id<JavaUtilIterator> iter__ = [((id<JavaLangIterable>) nil_chk(screens)) iterator];
      while ([((id<JavaUtilIterator>) nil_chk(iter__)) hasNext]) {
        TripleplayGameScreen *screen = [((id<JavaUtilIterator>) nil_chk(iter__)) next];
...
withTripleplayGameScreenStack_Transition:(id<TripleplayGameScreenStack_Transition>)trans {
  ...
  while ([((id<JavaUtilList>) nil_chk(_screens_)) size] > 1 && [((id<JavaUtilList>) nil_chk(_screens_)) getWithInt:1] != newTopScreen) {
    [self justRemoveWithTripleplayGameScreen:[((id<JavaUtilList>) nil_chk(_screens_)) getWithInt:1]];
  }
...
withTripleplayGameScreenStack_Transition:(id<TripleplayGameScreenStack_Transition>)trans {
  if ([((id<JavaUtilList>) nil_chk(_screens_)) isEmpty]) {
    ...
    TripleplayGameScreen *otop = [((id<JavaUtilList>) nil_chk(_screens_)) removeWithInt:0];
...
- (void)removeWithTripleplayGameScreenStack_Predicate:(id<TripleplayGameScreenStack_Predicate>)pred
             withTripleplayGameScreenStack_Transition:(id<TripleplayGameScreenStack_Transition>)trans {
  if ([((id<JavaUtilList>) nil_chk(_screens_)) size] > 1) {
    id<JavaUtilIterator> iter = [((id<JavaUtilList>) nil_chk(_screens_)) iterator];
    (void) [((id<JavaUtilIterator>) nil_chk(iter)) next];
    while ([((id<JavaUtilIterator>) nil_chk(iter)) hasNext]) {
      TripleplayGameScreen *screen = [((id<JavaUtilIterator>) nil_chk(iter)) next];
...
- (TripleplayGameScreen *)top {
  return [((id<JavaUtilList>) nil_chk(_screens_)) isEmpty] ? nil : [((id<JavaUtilList>) nil_chk(_screens_)) getWithInt:0];
}

- (TripleplayGameScreen *)findWithTripleplayGameScreenStack_Predicate:(id<TripleplayGameScreenStack_Predicate>)pred {
  {
    id<JavaUtilIterator> iter__ = [((id<JavaUtilList>) nil_chk(_screens_)) iterator];
    while ([((id<JavaUtilIterator>) nil_chk(iter__)) hasNext]) {
      TripleplayGameScreen *screen = [((id<JavaUtilIterator>) nil_chk(iter__)) next];
...
- (void)updateWithInt:(int)delta {
  ...
  else if (![((id<JavaUtilList>) nil_chk(_screens_)) isEmpty]) [((TripleplayGameScreen *) nil_chk([self top])) updateWithInt:delta];
}

- (void)paintWithPlaynCoreUtilClock:(id<PlaynCoreUtilClock>)clock {
  if (_transitor_ != nil) [_transitor_ paintWithPlaynCoreUtilClock:clock];
  else if (![((id<JavaUtilList>) nil_chk(_screens_)) isEmpty]) [((TripleplayGameScreen *) nil_chk([self top])) paintWithPlaynCoreUtilClock:clock];
}
...
- (void)justShowWithTripleplayGameScreen:(TripleplayGameScreen *)screen {
  [((id<PlaynCoreGroupLayer>) nil_chk([((id<PlaynCoreGraphics>) nil_chk([PlaynCorePlayN graphics])) rootLayer])) addAtWithPlaynCoreLayer:((TripleplayGameScreen *) nil_chk(screen)).layer withFloat:originX_ withFloat:originY_];
  @try {
...
- (void)hideWithTripleplayGameScreen:(TripleplayGameScreen *)screen {
  [((id<PlaynCoreGroupLayer>) nil_chk([((id<PlaynCoreGraphics>) nil_chk([PlaynCorePlayN graphics])) rootLayer])) removeWithPlaynCoreLayer:((TripleplayGameScreen *) nil_chk(screen)).layer];
  @try {
...
- (void)handleErrorWithJavaLangRuntimeException:(JavaLangRuntimeException *)error {
  [((TripleplayUtilLogger *) nil_chk([TripleplayGameLog log])) warningWithNSString:@"Screen choked" withNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ error } count:1 type:[IOSClass classWithClass:[NSObject class]]]];
}
...
- (void)init__ OBJC_METHOD_FAMILY_NONE {
  ...
  [((id<PlaynCorePointer>) nil_chk([PlaynCorePlayN pointer])) setEnabledWithBOOL:NO];
