...
- (id)initWithPlaynCoreImage:(id<PlaynCoreImage>)image {
  if ((self = [super init])) {
    imageLayer_ = [[TuxkidsTuxblocksCoreLayersImageLayerTintable alloc] initWithPlaynCoreImage:image];
    [TuxkidsTuxblocksCorePlayNObject centerImageLayerWithTuxkidsTuxblocksCoreLayersImageLayerTintable:imageLayer_];
  }
  return self;
}
  ...
  for (int aa = 0; aa < (int) [((IOSIntArray *) nil_chk(((TripleplayParticleParticleBuffer *) nil_chk(buffer)).alive)) count]; aa++) {
    ...
    for (int end = pp + 32; pp < end; pp++, ppos += TripleplayParticleParticleBuffer_NUM_FIELDS, mask <<= 1) {
      ...
      if (colorEffector != nil) {
        [((TuxkidsTuxblocksCoreLayersImageLayerTintable *) nil_chk(imageLayer_)) setTintWithInt:[colorEffector startColor] withInt:[colorEffector endColor] withFloat:1 - [colorEffector getPercWithFloatArray:data withInt:ppos withFloat:now]];
      }
      else {
        [((TuxkidsTuxblocksCoreLayersImageLayerTintable *) nil_chk(imageLayer_)) setTintWithInt:[PlaynCoreColor rgbWithInt:(int) (255 * [((IOSFloatArray *) nil_chk(data)) floatAtIndex:ppos + TripleplayParticleParticleBuffer_RED]) withInt:(int) (255 * [((IOSFloatArray *) nil_chk(data)) floatAtIndex:ppos + TripleplayParticleParticleBuffer_GREEN]) withInt:(int) (255 * [((IOSFloatArray *) nil_chk(data)) floatAtIndex:ppos + TripleplayParticleParticleBuffer_BLUE])]];
      }
      [((TuxkidsTuxblocksCoreLayersImageLayerTintable *) nil_chk(imageLayer_)) setAlphaWithFloat:[((IOSFloatArray *) nil_chk(data)) floatAtIndex:ppos + TripleplayParticleParticleBuffer_ALPHA]];
      (void) [((id<PlaynCoreSurface>) nil_chk(surface)) drawLayerWithPlaynCoreLayer:[((TuxkidsTuxblocksCoreLayersImageLayerTintable *) nil_chk(imageLayer_)) layerAddable]];
      rendered++;
