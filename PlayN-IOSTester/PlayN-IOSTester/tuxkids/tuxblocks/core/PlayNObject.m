...
+ (float)lerpWithFloat:(float)x0
  ...
  if ([JavaLangMath absWithFloat:x - x1] < snapDistance) x = x1;
  return x;
}
...
+ (float)lerpTimeWithFloat:(float)x0
                 ...
                 withFloat:(float)snapDistance {
  float perc = 1 - [PythagorasFFloatMath powWithFloat:base withFloat:dt];
  return [TuxkidsTuxblocksCorePlayNObject lerpWithFloat:x0 withFloat:x1 withFloat:perc withFloat:snapDistance];
}
...
+ (float)shiftToWithFloat:(float)x0
  ...
  float change = x1 - x0;
  change = [JavaLangMath minWithFloat:[JavaLangMath absWithFloat:change] withFloat:maxShift] * [JavaLangMath signumWithFloat:change];
  return x0 + change;
}
...
+ (void)centerImageLayerWithTuxkidsTuxblocksCoreLayersImageLayerTintable:(TuxkidsTuxblocksCoreLayersImageLayerTintable *)layer {
  if ([((TuxkidsTuxblocksCoreLayersImageLayerTintable *) nil_chk(layer)) image] != nil) {
    [((id<PlaynCoreImage>) nil_chk([((TuxkidsTuxblocksCoreLayersImageLayerTintable *) nil_chk(layer)) image])) addCallbackWithPlaynCoreUtilCallback:[[TuxkidsTuxblocksCorePlayNObject_$2 alloc] initWithTuxkidsTuxblocksCoreLayersImageLayerTintable:layer]];
  }
}
...
- (void)onSuccessWithId:(id<PlaynCoreImage>)result {
  [((TuxkidsTuxblocksCoreLayersImageLayerTintable *) nil_chk(val$layer_)) setOriginWithFloat:[((id<PlaynCoreImage>) nil_chk(result)) width] / 2 withFloat:[((id<PlaynCoreImage>) nil_chk(result)) height] / 2];
}
