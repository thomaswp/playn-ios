//
//  Generated by the J2ObjC translator.  DO NOT EDIT!
//  source: core/src/main/java/tuxkids/tuxblocks/core/layers/ImageLayerTintable.java
//
//  Created by Thomas on 7/26/13.
//

#ifndef _TuxkidsTuxblocksCoreLayersImageLayerTintable_H_
#define _TuxkidsTuxblocksCoreLayersImageLayerTintable_H_

@class JavaLangInteger;
@class JavaLangThrowable;
@class JavaUtilHashMap;
@protocol PlaynCoreGroupLayer;
@protocol PlaynCoreImage;
@protocol PlaynCoreImageLayer;
@protocol PlaynCoreLayer;
@protocol PlaynCorePointer_Listener;

#import "JreEmulation.h"
#include "playn/core/util/Callback.h"
#include "tuxkids/tuxblocks/core/PlayNObject.h"
#include "tuxkids/tuxblocks/core/layers/ImageLayerLike.h"

@interface TuxkidsTuxblocksCoreLayersImageLayerTintable : TuxkidsTuxblocksCorePlayNObject < TuxkidsTuxblocksCoreLayersImageLayerLike > {
 @public
  id<PlaynCoreGroupLayer> layer_;
  id<PlaynCoreImage> baseImage_;
  id<PlaynCoreImageLayer> base_;
  id<PlaynCoreImageLayer> top_;
  int tint__;
  JavaUtilHashMap *tintMap_;
}

@property (nonatomic, strong) id<PlaynCoreGroupLayer> layer;
@property (nonatomic, strong) id<PlaynCoreImage> baseImage;
@property (nonatomic, strong) id<PlaynCoreImageLayer> base;
@property (nonatomic, strong) id<PlaynCoreImageLayer> top;
@property (nonatomic, assign) int tint_;
@property (nonatomic, strong) JavaUtilHashMap *tintMap;

- (BOOL)useGL;
- (id<PlaynCoreGroupLayer>)parent;
- (float)width;
- (float)height;
- (id<PlaynCoreLayer>)layerAddable;
- (id<PlaynCoreImage>)image;
- (float)tx;
- (float)ty;
- (int)tint;
- (float)depth;
- (float)alpha;
- (BOOL)visible;
- (float)originX;
- (float)originY;
- (float)rotation;
- (void)setTxWithFloat:(float)tx;
- (void)setTyWithFloat:(float)ty;
- (void)setTranslationWithFloat:(float)x
                      withFloat:(float)y;
- (void)setOriginWithFloat:(float)x
                 withFloat:(float)y;
- (void)setImageWithPlaynCoreImage:(id<PlaynCoreImage>)image;
- (void)setScaleWithFloat:(float)scale_;
- (void)setScaleWithFloat:(float)scaleX
                withFloat:(float)scaleY;
- (void)setDepthWithFloat:(float)depth;
- (void)setVisibleWithBOOL:(BOOL)visible;
- (void)setAlphaWithFloat:(float)alpha;
- (void)setRotationWithFloat:(float)angle;
- (void)setSizeWithFloat:(float)width
               withFloat:(float)height;
- (void)setWidthWithFloat:(float)width;
- (void)setHeightWithFloat:(float)height;
- (id)init;
- (id)initWithPlaynCoreImage:(id<PlaynCoreImage>)image;
- (void)setTintWithInt:(int)color;
- (void)setTintWithInt:(int)baseColor
               withInt:(int)tintColor
             withFloat:(float)perc;
- (id<PlaynCoreImage>)getTintedImageWithJavaLangInteger:(JavaLangInteger *)color;
- (void)destroy;
- (void)addListenerWithPlaynCorePointer_Listener:(id<PlaynCorePointer_Listener>)pointerListener;
- (void)setInteractiveWithBOOL:(BOOL)interactive;
@end

@interface TuxkidsTuxblocksCoreLayersImageLayerTintable_$1 : NSObject < PlaynCoreUtilCallback > {
 @public
  TuxkidsTuxblocksCoreLayersImageLayerTintable *this$0_;
  int val$color_;
}

@property (nonatomic, strong) TuxkidsTuxblocksCoreLayersImageLayerTintable *this$0;
@property (nonatomic, assign) int val$color;

- (void)onSuccessWithId:(id<PlaynCoreImage>)result;
- (void)onFailureWithJavaLangThrowable:(JavaLangThrowable *)cause;
- (id)initWithTuxkidsTuxblocksCoreLayersImageLayerTintable:(TuxkidsTuxblocksCoreLayersImageLayerTintable *)outer$
                                                   withInt:(int)capture$0;
@end

@interface TuxkidsTuxblocksCoreLayersImageLayerTintable_$2 : NSObject < PlaynCoreUtilCallback > {
 @public
  TuxkidsTuxblocksCoreLayersImageLayerTintable *this$0_;
  int val$baseColor_;
  int val$tintColor_;
}

@property (nonatomic, strong) TuxkidsTuxblocksCoreLayersImageLayerTintable *this$0;
@property (nonatomic, assign) int val$baseColor;
@property (nonatomic, assign) int val$tintColor;

- (void)onSuccessWithId:(id<PlaynCoreImage>)result;
- (void)onFailureWithJavaLangThrowable:(JavaLangThrowable *)cause;
- (id)initWithTuxkidsTuxblocksCoreLayersImageLayerTintable:(TuxkidsTuxblocksCoreLayersImageLayerTintable *)outer$
                                                   withInt:(int)capture$0
                                                   withInt:(int)capture$1;
@end

#endif // _TuxkidsTuxblocksCoreLayersImageLayerTintable_H_
