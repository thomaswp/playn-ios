...
- (int)timeUntilNextRound {
  ...
  return [((JavaLangInteger *) nil_chk([((id<JavaUtilList>) nil_chk(waitTimes_)) getWithInt:0])) intValue] - timer_;
}

- (void)startNextRound {
  if (currentRound_ != nil) return;
  if ([((id<JavaUtilList>) nil_chk(waitTimes_)) size] == 0) return;
  timer_ = [((JavaLangInteger *) nil_chk([((id<JavaUtilList>) nil_chk(waitTimes_)) getWithInt:0])) intValue];
}
...
- (TuxkidsTuxblocksCoreDefenseWalkerWalker *)updateWithInt:(int)delta {
  ...
  timer_ += delta;
  if (currentRound_ == nil && [((id<JavaUtilList>) nil_chk(waitTimes_)) size] > 0 && timer_ >= [((JavaLangInteger *) nil_chk([((id<JavaUtilList>) nil_chk(waitTimes_)) getWithInt:0])) intValue]) {
    currentRound_ = [((id<JavaUtilList>) nil_chk(rounds_)) removeWithInt:0];
